...
void process_connections(forwarder_t* forwarder) {
	...
	while (list != NULL) {
		if (list->content != NULL) {
			...
			if (list->content->eof && list->content->send_offset != list->content->recv_offset) {
				if (list->next != NULL) {
					list = list->next;
	...
	if (removed) {
		...
		while (list != NULL) {
			if (list->content != NULL) {
				forwarder->maxfd = max(forwarder->maxfd, max(list->content->connected_fd, list->content->coupled_fd));
			}
			list = list->next;
		}
	}
